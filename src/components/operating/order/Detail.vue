<template>
    <div id="OrderDetail">
        ...
        <div class="order-detail-orderinfo mt-3">
            <el-row>
              ...
              <el-button class="pr-3" type="success" v-if="order.state===1" size="mini" round>接单</el-button v-if="order.state===1">
            ...
            <div class="py-3 pl-4">
              <el-row class="base-info">
                <el-col :span="6" v-for="(info, index) in baseinfo" class="d-ib mb-2" :index="(index + 1).toString()" :key="index">
                  <div>{{ info.name }} :</div>
                  <div>{{ info.value }} </div>
                </el-col>
              </el-row>
  ...
  export default {
    ...
    created() {
      this.getOrderDetail(this.$store.state.orderId)
    }
  };
  </script>
